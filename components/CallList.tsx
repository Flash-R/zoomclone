...
function CallList({type}: {type: 'ended' | 'upcoming' | 'recordings'}) {
    ...
    // a function that returns the exact calls depending on the page
    const getCalls = () => {
        switch (type) {
            case 'ended':
                return endedCalls;
    ...
    // fetching recordings from meetings
    useEffect(() => {
    ...
    <div className='grid grid-cols-1 gap-5 xl:grid-cols-2'>
        {
            calls && calls.length > 0 ? calls.map((meeting: Call | CallRecording) =>(
                ...
                buttonIcon1 ={
                    type === 'recordings' ? '/icons/play.svg' : ''
                }
                handleClick ={type ==='recordings' ? () => router.push(meeting.url) : () => router.push(
